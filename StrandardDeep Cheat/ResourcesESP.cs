...
{
    public class ResourcesHARVESTPLANTESP : MonoBehaviour
    {
        private void Start()
        {
        ...
        private void OnGUI()
        {
            if (Menu.LootESP == true)
            {
                ...
                {
                    IEnumerable<InteractiveObject_HARVESTPLANT> playerProxies = FindObjectsOfType<InteractiveObject_HARVESTPLANT>();
                    foreach (InteractiveObject_HARVESTPLANT playerProxy in playerProxies)
                    {
                        ...
                        if (posScreen.z > 0 & posScreen.y < Screen.width - 2)
                        {
                            ...
                            if (dist < Menu.LoopDist)
                            {
                                ...
                                Render.DrawString(new Vector2(posScreen.x, posScreen.y), ("  Harvest Plant" + string.Format(" [{0:0}m]", (object)dist)), Color.green);
                            }
                        }
                        SleepFor(0.300f);
                    }
