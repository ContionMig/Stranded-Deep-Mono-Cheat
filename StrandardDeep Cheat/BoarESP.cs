...
{
    public class BoarESP : MonoBehaviour
    {
        private void Start()
        {
        ...
        private void OnGUI()
        {
            if (Menu.EntityESP == true)
            {
                if (Menu.BoarESP)
                {
                    IEnumerable<InteractiveObject_BOAR> playerProxies = FindObjectsOfType<InteractiveObject_BOAR>();
                    foreach (InteractiveObject_BOAR playerProxy in playerProxies)
                    {
                        ...
                        if (posScreen.z > 0 & posScreen.y < Screen.width - 2)
                        {
                            ...
                            if (dist < Menu.EntityDist)
                            {
                                ...
                                Render.DrawString(new Vector2(posScreen.x, posScreen.y), ("   Boar" + string.Format(" [{0:0}m]", (object)dist)), Color.green);
                            }
                        }
                        SleepFor(0.300f);
                    }
