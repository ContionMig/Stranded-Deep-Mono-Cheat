...
{
    public class MiningRocksESP : MonoBehaviour
    {
        private void Start()
        {
        ...
        private void OnGUI()
        {
            ...
            {
                if (Menu.MiningRocksESP)
                {
                    IEnumerable<InteractiveObject_MININGROCK> playerProxies = FindObjectsOfType<InteractiveObject_MININGROCK>();
                    foreach (InteractiveObject_MININGROCK playerProxy in playerProxies)
                    {
                        ...
                        if (posScreen.z > 0 & posScreen.y < Screen.width - 2)
                        {
                            ...
                            if (dist < Menu.LoopDist)
                            {
                                ...
                                Render.DrawString(new Vector2(posScreen.x, posScreen.y), (" Mining Rocks" + string.Format(" [{0:0}m]", (object)dist)), Color.green);
                            }
                        }
                        SleepFor(0.300f);
                    }
