...
{
    public class FuelCanESP : MonoBehaviour
    {
        private void Start()
        {
        ...
        private void OnGUI()
        {
            if (Menu.LootESP == true)
            {
                if (Menu.FuelCanESP)
                {
                    IEnumerable<InteractiveObject_FUELCAN> playerProxies = FindObjectsOfType<InteractiveObject_FUELCAN>();
                    foreach (InteractiveObject_FUELCAN playerProxy in playerProxies)
                    {
                        ...
                        if (posScreen.z > 0 & posScreen.y < Screen.width - 2)
                        {
                            ...
                            if (dist < Menu.LoopDist)
                            {
                                ...
                                Render.DrawString(new Vector2(posScreen.x, posScreen.y), ("   Fuel Can" + string.Format(" [{0:0}m]", (object)dist)), Color.green);
                            }
                        }
                        SleepFor(0.300f);
                    }
